...
add_action( 'plugins_loaded', 'run_edt_psh_gateway' );

function run_edt_psh_gateway() {

    if( ! class_exists( 'EDT_PardakhtShahr_Gateway' ) && class_exists( 'Easy_Donations_Gateway' ) ) {
        
        
        
        class EDT_PardakhtShahr_Gateway extends Easy_Donations_Gateway {
            ...
            public function gateway_settings_fields() {
                $gateway_id = 'edt_psh_gateway';
                $gtw_data = edt_ins()->options->get_option( $gateway_id );
                $setting_fields = array(
                            array(
                                'id'      => 'pardakhtshahr_api',
                                'name'    => '[pardakhtshahr_api]',
                                'type'    => 'text',
                                'text'    => 'کد API دریافتی از شهر پرداخت را وارد نمایید.',
                                'value'   => ( isset( $gtw_data['pardakhtshahr_api'] ) ) ? $gtw_data['pardakhtshahr_api'] : ''
                            )
                        );
                $this->add_gtw_setting( $gateway_id, 'شهر پرداخت', $setting_fields );
                
            }
            
            public function before_send( $payment ) {
                
                $target_url = 'http://pardakhtshahr.ir/pay/webservice/?wsdl';

                $gtw_data = edt_ins()->options->get_option( 'edt_psh_gateway' );
                
                $API = ( isset( $gtw_data['pardakhtshahr_api'] ) ) ? $gtw_data['pardakhtshahr_api'] : '';
                ...
                if($res > 0 && is_numeric($res)){
                    $payment_url = 'http://pardakhtshahr.ir/pay/payment/';
                    
                    Header( "Location: $payment_url" . $res );
                    die();
                }
                else{
                    $this->add_message( 'خطا در ارسال به شهر پرداخت کد خطا:' . $res . ' علت آن به این شرح است: ' .  ((isset($this->errorCode[$res])) ? $this->errorCode[$res] : 'کد خطا قابل تفسیر نیست.' ), 'error' );
        ...
        edt_ins()->gateways->register_gateway( 'edt_psh_gateway', 'شهر پرداخت', 'EDT_PardakhtShahr_Gateway' );
    }
}